...
const filteredReports = computed(() => {
  ...
  if (searchQuery.value.trim() !== '') {
    ...
    reports = reports.filter(
      (report) =>
        (report.idLaporan && report.idLaporan.toLowerCase().includes(lowerCaseQuery)) ||
...
<template>
  <div>
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm mb-8 flex flex-col md:flex-row gap-4">
      <div class="flex-1">
        <label for="filter-status" class="block text-sm font-semibold text-gray-700 mb-1"
          >Filter Status</label
        >
        <select
          id="filter-status"
      ...
      <div class="flex-1">
        <label for="filter-satwa" class="block text-sm font-semibold text-gray-700 mb-1"
          >Filter Jenis Satwa</label
        >
        <select
          id="filter-satwa"
          v-model="selectedSatwa"
          class="w-full px-4 py-2 border rounded-lg focus:ring-brand-green-light focus:border-brand-green-light"
        >
    ...
    <div
      ...
      class="bg-white p-8 rounded-xl shadow-lg text-center text-gray-500"
    >
    ...
    <div v-else>
      <div class="space-y-4">
        <div
          v-for="laporan in paginatedReports"
          :key="laporan.id"
          class="bg-white p-5 rounded-xl border border-gray-200 hover:shadow-xl hover:border-brand-accent transition-all duration-300 cursor-pointer"
          @click="emit('view-detail', laporan.id)"
        >
          <div class="flex flex-col md:flex-row justify-between items-start">
            <div class="flex-grow mb-4 md:mb-0">
              <p class="font-bold text-sm text-gray-500 mb-1">{{ laporan.idLaporan }}</p>
