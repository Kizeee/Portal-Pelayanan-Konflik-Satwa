    ...
    <div v-if="user" class="bg-white p-8 rounded-2xl shadow-lg mt-6">
      ...
      <div class="bg-gray-50 p-4 rounded-lg">
        ...
        <div class="flex items-center space-x-4">
          <select id="status-select" v-model="currentStatus" class="w-full md:w-1/3 px-4 py-2 border rounded-lg">
            <option>Diterima</option><option>Diproses</option><option>Selesai</option><option>Tidak Valid</option>
          </select>
          <button @click="updateLaporanStatus" :disabled="isSubmitting" class="bg-blue-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-blue-700 disabled:bg-gray-400">{{ isSubmitting ? 'Menyimpan...' : 'Simpan' }}</button>
        </div>
        <div class="mt-4">
